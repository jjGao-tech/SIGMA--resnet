...
weight_path = "./pth/CNN_7_32_noise_theta_DR_BNN_all_0.06.pth"        # ← 改成你的 CNN state_dict 路径
out_dir     = "./csv/eval_CNN_mc_all_0.06_model_train"          # 输出目录（多CSV+最终聚合）
...
#    返回：img, lbl(4*K_max), fname, coords, base_fn（与原脚本一致）
# -----------------------------
...
# -----------------------------
# 3) 模型（替换为你的 CNN，输出维度与训练一致：2*4*K_max）
# -----------------------------
...
# -----------------------------
# 6) 论文同款：直接 train() 启用 MC（Dropout & BN 都在训练态）
# -----------------------------
def enable_mc_dropout_like_paper(m: nn.Module):
    m.train()  # Dropout 生效，BN 使用 batch 统计并更新 running stats
    for p in m.parameters():
        p.requires_grad_(False)  # 推理不需要梯度

enable_mc_dropout_like_paper(model)
print(f"Running MC Dropout (paper style, model.train()) with T={T_MC}")

# -----------------------------
# 7) MC 前向：重复 T_MC 次，每次写一个 CSV（pred_dp_XXXX.csv）
# -----------------------------
for t in range(T_MC):
...
# -----------------------------
# 8) 聚合：读取所有 per-pass CSV，按 N(mu, sigma) 采样并求最终均值/方差
# -----------------------------
...
with open(final_csv, "w", newline='') as f:
    ...
    # RMSE 行（与 abs_err_* 对齐）—— 每个参数（8 列）的最终 RMSE
